...
syn keyword pentadactylCommand loadplugins lpl gr[oup] ! run Clistk[eys] Clk
    ...
    \ ab[breviate] addo[ns] ao au[tocmd] ba[ck] bd[elete] bw[ipeout] bun[load]
    \ tabc[lose] blistk[eys] blk bm[ap] bma[rk] bmarks bno[remap] b[uffer]
    \ buffers files ls tabs bunm[ap] ca[bbreviate] cd chd[ir] clistk[eys] clk
    \ cm[ap] cno[remap] colo[rscheme] com[mand] comp[letions] contexts cookies
    \ ck cuna[bbreviate] cunm[ap] delbm[arks] delc[ommand] delg[roup]
    \ delmac[ros] delm[arks] delqm[arks] dels[tyle] dia[log] doautoa[ll]
    \ do[autocmd] downl[oads] dl ec[ho] echoe[rr] echom[sg] el[se] elsei[f] elif
    \ em[enu] en[dif] fi exe[cute] exta[dd] extde[lete] extd[isable] exte[nable]
    \ exto[ptions] extp[references] extr[ehash] extt[oggle] extu[pdate] feedkeys
    \ fk fini[sh] flashplay flp flashstop fls flashtoggle flt fo[rward] fw
    \ frameo[nly] ha[rdcopy] h[elp] helpa[ll] hi[ghlight] hist[ory] hs
    \ ia[bbreviate] if ilistk[eys] ilk im[ap] ino[remap] iuna[bbreviate]
    \ iunm[ap] javas[cript] js ju[mps] keepa[lt] let listc[ommands] lc
    \ listk[eys] lk listo[ptions] lo macros map ma[rk] marks mes[sages]
    ...
    \ nunm[ap] o[pen] pa[geinfo] pagest[yle] pas pin[tab] pref[erences] prefs
    \ pw[d] qma[rk] qmarks q[uit] quita[ll] qa[ll] redr[aw] reh[ash] re[load]
...
syn keyword pentadactylOption activate act altwildmode awim autocomplete au
    ...
    \ fbwhitelist fbw fileencoding fenc findcase fc followhints fh guioptions go
    \ helpfile hf hintinputs hin hintkeys hk hintmatching hm hinttags ht
    \ hinttimeout hto history hi iskeyword isk loadplugins lpl mapleader ml
    \ maxitems messages msgs newtab nextpattern pageinfo pa passkeys pk
    \ passunknown popups pps previouspattern runtimepath rtp sanitizeitems si
    \ sanitizeshutdown ss sanitizetimespan sts scroll scr shell sh shellcmdflag
    \ shcf showmode smd showstatuslinks ssli showtabline stal strictfocus sf
    \ suggestengines timeoutlen tmol titlestring urlseparator urlsep us verbose
    \ vbs wildanchor wia wildcase wic wildignore wig wildmode wim wildsort wis
    \ wordseparators wsp
    \ contained nextgroup=pentadactylSetMod

let s:toggleOptions = ["banghist", "bh", "errorbells", "eb", "exrc", "ex",
    \ "flashblock", "fb", "fullscreen", "fs", "hlfind", "hlf", "incfind", "if",
    \ "insertmode", "im", "jsdebugger", "jsd", "more", "online", "private",
    \ "pornmode", "timeout", "tmo", "usermode", "um", "visualbell", "vb"]
...
syn region pentadactylJavaScript start="\%(^\s*\%(javascript\|js\)\_s\+\)\@<=" end="$" contains=@javascriptTop keepend oneline
...
let s:cssRegionStart = '\%(^\s*sty\%[le]!\=\_s\+\%(-\%(n\|name\)\%(\_s\+\|=\)\_S\+\_s\+\)\=[^-]\_S\+\_s\+\)\@<='
execute 'syn region pentadactylCss start="' . s:cssRegionStart . '" end="$" contains=@cssTop keepend' 
execute 'syn region pentadactylCss matchgroup=pentadactylCssDelimiter'
    \ 'start="' . s:cssRegionStart . '<<\_s*\z(\h\w*\)"hs=s+2 end="^\z1$" contains=@cssTop fold'
...
syn region pentadactylString start="\z(["']\)" end="\z1" skip="\\\\\|\\\z1"

syn match pentadactylLineComment +^\s*".*$+ contains=pentadactylTodo,@Spell

syn match pentadactylCont +\n\s*\\\s*+ containedin=pentadactylString,pentadactylCss,pentadactylJavaScript
...
hi def link pentadactylCont                  Normal
